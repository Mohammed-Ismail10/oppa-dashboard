...
const rows = [
  { id: 1, mid: 25, type: 'متلقي', level: 10, exp: 100000, Img: "https://fakestoreapi.com/img/61sbMiUnoGL._AC_UL640_QL65_ML3_.jpg" },
  { id: 2, mid: 25, type: 'متلقي', level: 10, exp: 100000, Img: "https://fakestoreapi.com/img/61sbMiUnoGL._AC_UL640_QL65_ML3_.jpg" },
];
...
export default function TableListLevels() {
  const columns = [
    {
    ...
    {
      dataField: 'level', //must be same name of property in row which come from api
      text: '',
      headerFormatter: () => <span className='py-2 badge text-main rounded fs15 border w-75'>
    ...
    },
    {
      ...
      headerFormatter: () => <span className='py-1 badge text-main rounded fs15 border w-75'>
        <img className='me-1 p-1' src={star} alt="star" width={23} />
        نقاط الخبرة
      </span>,
      classes: 'text-main fs15 pt-3 px-0',
    },
  ...
  let [currentPage, setCurrentPage] = useState(() => {
    const storedPage = localStorage.getItem('currentPageListLevels');
    return storedPage ? parseInt(storedPage) : 0;
  });

  useEffect(() => {
    localStorage.setItem('currentPageListLevels', currentPage);
  }, [currentPage]);
  ...
  let { data, isLoading, refetch } = useQuery('level', getData, {
    cacheTime: 60000,
